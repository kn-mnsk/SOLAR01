...
#include <mylibrary/handleerrors1.h>
	...
	public:
		...
		Text(std::string fontname) {
			...
			if (!success) {
				...
				ErrorExit(L"ERROR => TextRendering::Text class: Failed to generate instance");
			}
	...
	private:
		...
		bool loadFont(std::string filepath)
		{
			if (FT_Init_FreeType(&ft))
			{
				...
				ErrorExit(L"ERROR => TextRendering::Text class   FREETYPE: Could not init FreeType Library");
			}
			...
			if (FT_New_Face(ft, filepath.c_str(), 0, &face)) {
				...
				ErrorExit(L"ERROR => TextRendering::Text class  FREETYPE: Failed to load font");
			}
			...
			for (unsigned char c = 0; c < 128; c++)
			{
				...
				if (FT_Load_Char(face, c, FT_LOAD_RENDER))
				{
					...
					ErrorExit(L"ERROR => TextRendering::Text class  FREETYTPE: Failed to load Glyph");
				}
				// generate texture
