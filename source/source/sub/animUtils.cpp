...
// determined.
///////////////////////////////////////////////////////////////////////////////
glm::vec3 Gil::slerp(const glm::vec3& from, const glm::vec3& to, float alpha, AnimationMode mode)
{
    // re-compute alpha
    ...
    float cosine = glm::dot(from, to) / (from.length() * to.length());
...
// determined.
///////////////////////////////////////////////////////////////////////////////
glm::quat Gil::slerp(const glm::quat& from, const glm::quat& to, float alpha, AnimationMode mode)
{
    // re-compute alpha
    float t = interpolate(0.0f, 1.0f, alpha, mode);
    ...
    if(1.0f - dot < 0.001f)
    {
        ...
        return glm::quat{ from + (to - from) * t };
    }
    ...
    else if(fabs(1 + dot) < 0.001f) // dot ~= -1
    {
        ...
        v1 = glm::normalize(glm::vec3{ from.x, from.y, from.z});
        //v1.normalize();
       ...
       // v2 = v1.cross(up); // orthonormal to v1
        v2 = glm::normalize(glm::cross(v1, up)); // orthonormal to v1
        //v2.normalize();
        ...
        glm::vec3 v3 = v1 * cosf(angle) + v2 * sinf(angle);
        //return Quaternion(0, v3.x, v3.y, v3.z);
        return glm::quat{ 0.0f, v3.x, v3.y, v3.z };
    }
